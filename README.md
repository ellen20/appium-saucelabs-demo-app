...
├── .github/
│ └── workflows/
│ └── android-e2e.yml # CI pipeline (see known limitation below)
...
A GitHub Actions workflow (`.github/workflows/android-e2e.yml`) is configured to run the Android E2E suite automatically on push and pull request to `main`, using [`reactivecircus/android-emulator-runner`](https://github.com/ReactiveCircus/android-emulator-runner) to boot a hardware-accelerated Android emulator in CI.

**Known limitation:** GitHub-hosted macOS runners do not reliably support the hardware virtualization (`HVF`) required for Android emulator acceleration — this was confirmed with a `HVF error: HV_UNSUPPORTED` failure across multiple runner versions (`macos-latest`, `macos-14`) and system image architectures (`x86_64`, `arm64-v8a`). This is a documented infrastructure constraint of nested virtualization on GitHub's hosted runners, not an issue with this project's test logic or configuration — the full suite is verified passing via local execution against a physical Android emulator (see [Real Bugs Found & Fixed](#-real-bugs-found--fixed-during-development) for details on the debugging process).

**Planned resolution:** migrate CI execution to Sauce Labs' cloud device grid (`wdio.sauce.conf.ts`, already scaffolded), which runs tests against real cloud-hosted devices and avoids the local hardware-acceleration dependency entirely.
...
- [x] GitHub Actions CI/CD pipeline configured (blocked by GitHub-hosted runner hardware limitations — see CI/CD section)
- [ ] Migrate CI execution to Sauce Labs cloud device grid
